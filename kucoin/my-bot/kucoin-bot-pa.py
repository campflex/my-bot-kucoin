...
def calculate_order_price(trading_pair, increased_price):
    """
    Determines the price at which to place both buy and sell orders based on the current order book.
    :param trading_pair: The trading pair (e.g., 'BTC-USDT').
    :param increased_price: The proposed increased price.
    ...
    # Extract all existing order prices from the order book
    existing_prices = set(float(order[0]) for order in asks)

    # Check if the increased price already exists in the order book
    if increased_price in existing_prices:
        print(f"Avoiding order placement as the chosen price {increased_price} already exists in the order book!")
        return None
...
def trade_loop(ttrading_pair, quantity_min, quantity_max, interval, num_trades):
    """
    ...
    for i in range(num_trades):
        ...
        # Calculate the price increase spread for buy/sell
        increase_price_spread = current_price * (increase_percent / 100)
        buy_sell_price = increase_price_spread + current_price
        # Format sell_price to match the required precision
        buy_sell_price = round(buy_sell_price, 6)

         # Check if the chosen price is the same as the previous one
        if buy_sell_price == current_price:
            print(f"\033[91mChosen price {buy_sell_price} is the same as the current price. Skipping this trade cycle.\033[0m")
            continue  
        # Calculate a single price for both buy and sell orders
        chosen_price = calculate_order_price(trading_pair, buy_sell_price)
        ...
        print(f"Waiting for {interval} seconds before checking order statuses...")
        time.sleep(interval)  # Adjust buffer time as necessary
...
parser.add_argument('--increase_percent', type=float, required=True,
                    help='Price increase percentage, e.g., 0.07 for 0.07%')
...
increase_percent= args.increase_percent  # Use the argument value
