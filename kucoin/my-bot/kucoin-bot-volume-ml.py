...
def calculate_order_price(trading_pair, current_price, avoid_sell_on_bid_order_match, avoid_buy_on_ask_order_match, allow_below_current_price):
    ...
    # Adjust current_price if outside bid-ask range
    if current_price > closest_ask_price:
        print(f"\033[95mCurrent price {current_price} is above closest ask {closest_ask_price}. Adjusting to closest ask.\033[0m")
        current_price = closest_ask_price
    elif current_price < closest_bid_price:
        print(f"\033[95mCurrent price {current_price} is below closest bid {closest_bid_price}. Adjusting to closest bid.\033[0m")
        current_price = closest_bid_price

    # Now calculate adjusted lower bound safely
